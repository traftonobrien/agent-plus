---
name: agent-plus-tiered-model-router
description: Choose bounded Agent+ model roles for an authorized formal chain. Not mandatory paperwork for routine maintenance.
---
...
Use before assigning model roles in an authorized formal chain. Routine reversible work uses
the lightweight route in `AGENTS.md` and needs no complete task packet or worker.
...
routine search or counts. Apply the formal evaluator small-correction rule in `AI_WORKFLOW.md` where eligible.
Stop on maker, substantive evaluator, or reviewer `BLOCK`; never auto-repair after a
