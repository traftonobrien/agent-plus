---
name: agent-plus-model-review
description: Compare prospective Agent+ model-routing and workflow outcomes. Not global session collection or model-quality certification.
---
...
For a new-model comparison, preserve the owner-selected model and effective reasoning effort.
Compare the same bounded tasks before and after instruction changes, then vary one setting at a time.
Record acceptance, interruptions, elapsed time, tool calls, retries, and reviewer corrections when observed.
Missing metrics stay unavailable. Separate context-size measurements and policy scenarios from actual
end-to-end model performance. Use prospective records; do not infer outcomes from old transcripts.
Use the existing outcome audit only when its closed record schema fits the measured work.
